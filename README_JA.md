...
[English](./README_EN.md) | 日本語 | [简体中文](./README_CN.md)|[FakeChinese](./README_FZH.md)
...
```bash
# 期限の一括編集（タスク5から8までの期限を今月の22日に設定）
jodo -e 5to8 -t 22
jodo -e 5-8 -t 22  # 同等の書き方

# 複数のタスクを一括で完了としてマーク
jodo -c 1-5    # タスク1から5までを完了としてマーク
jodo -c 1to5   # 同等の書き方

# 複数のタスクの完了マークを一括で解除
jodo -u 1c-5c    # 完了済みのタスク1cから5cまでの完了マークを解除
jodo -u 1cto5c   # 同等の書き方

# 複数のタスクを一括で削除
jodo -d 1-3    # タスク1から3までを削除
jodo -d 1to3   # 同等の書き方
```
